					...
					enum led_brightness value)
{
	...
	if (!bl_lvl && value)
		bl_lvl = 1;
printk(KERN_EMERG "backlight API %d \n", bl_lvl);
						...
						struct mdp_bl_scale_data *data)
{
	...
	pr_debug("%s: update scale = %d, min_lvl = %d\n", __func__, bl_scale,
								bl_min_lvl);
printk(KERN_EMERG "mdp_bl_scale_config bl_scale=%d, bl_min_lvl=%d, backlight = %d \n", bl_scale, bl_min_lvl, mfd->bl_level);
...
void msm_fb_set_backlight(struct msm_fb_data_type *mfd, __u32 bkl_lvl)
{
	...
	if (!mfd->panel_power_on || !bl_updated) {
		...
		printk(KERN_EMERG "backlight return %d \n", temp);
		return;
	} else {
		unset_bl_level = 0;
	}
	...
	if ((pdata) && (pdata->set_backlight)) {
		...
		printk(KERN_EMERG "set backlight %d", mfd->bl_level);
...
/*[PLATFORM]-Mod-BEGIN by TCTNB.YJ, 2013/1/14, add sync between LCD init and LCD backlight */	
	if( (unset_bl_level && !bl_updated) || !pan_display_status ){
		...
		if ((pdata) && (pdata->set_backlight)) {
			...
			pdata->set_backlight(mfd);
			printk(KERN_EMERG "pan display backlight %d \n", mfd->bl_level);
