			      ...
			      enum wpa_states state)
{
	...
	if (wpa_s->wpa_state == WPA_COMPLETED &&
	    (state == WPA_DISCONNECTED || state == WPA_ASSOCIATED)) {
		/* do not reset the time, until new connection */
		ccx_apinfo_update_time(wpa_s->wpa);
	}
#endif /* CONFIG_CCX */
	...
	if (state == WPA_COMPLETED && wpa_s->new_connection) {
		...
		if (wpa_key_mgmt_cckm(wpa_s->key_mgmt)) {
			ccx_apinfo_set(wpa_s->wpa, wpa_s->bssid,
				       wpa_s->current_bss ?
				       wpa_s->current_bss->freq : 0,
				       wpa_s->current_ssid->ssid,
				       wpa_s->current_ssid->ssid_len);
		} else {
			ccx_apinfo_invalidate(wpa_s->wpa);
		}
#endif /* CONFIG_CCX */
			      ...
			      u8 *wpa_ie, size_t *wpa_ie_len)
{
	...
	if (bss_rsn && (ssid->proto & WPA_PROTO_RSN) &&
		...
		proto = WPA_PROTO_WPA;
#ifdef CONFIG_WAPI
	} else if ((ssid->proto & WPA_PROTO_WAPI) && bss &&
				 ...
				 int reason_code)
{
	...
	union wpa_event_data event;
        //[BUGFIX]-Add-BEGIN by TCTNB.Xiushui.Ye,12/11/2012, 376971,
        //merge QuIC patches, [PATCH] wpa_supplicant: Use wpa_drv_{disassociate,deauthenticate}
        int zero_addr = 0;
	...
	if (addr) {
        ...
        //[BUGFIX]-Add-END by TCTNB.Xiushui.Ye
#ifdef CONFIG_WAPI
	if (wpa_s->key_mgmt == WPA_KEY_MGMT_WAPI_PSK ||
	    wpa_s->key_mgmt == WPA_KEY_MGMT_WAPI_CERT)
		wapi_supplicant_disassociate(wpa_s, reason_code);
